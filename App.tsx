...
import TestBench from './components/TestBench'; // Import TestBench
import AgentDashboard from './components/AgentDashboard'; // Import AgentDashboard
import SafeComponent from './components/SafeComponent'; // Import SafeComponent
import { SafeModeProvider, useSafeMode } from './contexts/SafeModeContext'; // Import Context
import { ColosseumProvider } from './contexts/ColosseumContext'; // Import Colosseum Context
...
import { Layers, FileCode2, Github, AlertTriangle, UploadCloud, Activity, BrainCircuit } from 'lucide-react';
...
const AppContent: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-200 font-sans selection:bg-blue-500/30">
      ...
      {/* Test Bench Modal */}
      {showTestBench && <TestBench onClose={() => setShowTestBench(false)} />}
      
      {/* Agent Colosseum Modal */}
      {showColosseum && <AgentDashboard onClose={() => setShowColosseum(false)} fullContext={fullOutput} />}
      ...
      <nav className="border-b border-slate-800 bg-slate-950/50 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
             ...
             {/* Safe Mode Diagnostics Trigger */}
             <button 
                onClick={() => setShowTestBench(true)}
                className="flex items-center gap-2 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 rounded-full border border-slate-700 text-xs font-medium text-slate-300 transition-colors"
                title="Open Test Framework & Safe Mode"
             >
